...
#INSTALLS NECESSARY PACKAGES
 install.packages(c('hms','lubridate','tidytext','tm','wordcloud','igraph','flexdashboard'));
 install.packages(c('glue','networkD3','rtweet','plyr','stringr','ggplot2','knitr'));
 install.packages(c('ggeasy','plotly','dplyr','magrittr','tidyverse','emoji','sjmisc'));
 
#AFTER THE RUNNING INSTALL PACKAGES DELETE LINES 3 TO 6 SO YOU CAN RUN THE RMD FILE WITH NO ISSUE
...
tweets2 <- search_tweets(Lookup, n = 1000,include_rts = FALSE, type = "recent", max_id = tweets, lang = "en", fileEncoding = "UTF-8", retryonratelimit = TRUE)
...
#########################################################################################################################################################  
  ...
  neg.words = c(negative,'wtf','epicfail', 'arrest', 'shit', 'corruption','corrupt', 'bullshit', 'conspiracy' )
...
#########################################################################################################################################################    
 ...
 strippedTweets <-  strippedTweets%>%
  ...
  words<- strippedTweets %>% 
    with(wordcloud(word, n, scale = c(1.6,.6), random.order = F, max.words = 100, color =brewer.pal(8, "Dark2"), random.color = TRUE))
